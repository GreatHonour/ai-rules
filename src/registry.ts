...
import type { PackageKind, RegistryIndex, RegistryPackage, RegistryPackageVersion } from './types.js';
...
export function parseRegistryIndex(contents: string): RegistryIndex {
  ...
  const packages = value.packages.map((packageValue, packageIndex): RegistryPackage => {
    ...
    const versions = packageValue.versions.map((versionValue, versionIndex): RegistryPackageVersion => {
      assertRecord(versionValue, `versions[${versionIndex}]`);
      if (
        typeof versionValue.version !== 'string' || semver.valid(versionValue.version) === null ||
