...
function mergeResourceChanges(
  ...
  const mergedChanges = new Map<string, { readonly kind: ResourceKind; readonly name: string; readonly changes: Set<ResourceChangeType> }>();
  ...
  return [...mergedChanges.values()].map((resource) => {
    const change = resource.changes.size === 1
      ? resource.changes.values().next().value ?? 'modified'
      : 'modified';
...
function getResourcePath(resource: Omit<ChangedResource, 'change'>): string {
  return resource.kind === 'rules'
    ? `.agents/rules/${resource.name}.md`
    : `.agents/skills/${resource.name}`;
}

/** 判断资源在工作树中是否存在。 */
async function resourceExistsInWorkingTree(
...
/** 判断资源在当前 index 中是否存在。 */
async function resourceExistsInIndex(
  workspacePath: string,
  resource: Omit<ChangedResource, 'change'>,
): Promise<boolean> {
...
export async function detectWorkingResourceChanges(workspacePath: string): Promise<readonly ChangedResource[]> {
  const diffOutput = await runGit(workspacePath, [
    'diff', '--name-status', '--no-renames', '-z', 'HEAD', '--', '.agents/rules', '.agents/skills',
  ]);
  const untrackedOutput = await runGit(workspacePath, [
    'ls-files', '--others', '--exclude-standard', '-z', '--', '.agents/rules', '.agents/skills',
  ]);
  const trackedChanges = parseNameStatus(diffOutput);
  const untrackedChanges = untrackedOutput.split('\0')
  ...
  return Promise.all(resources.map(async ({ kind, name }) => {
...
export async function detectStagedResourceChanges(workspacePath: string): Promise<readonly ChangedResource[]> {
  const diffOutput = await runGit(workspacePath, [
    'diff', '--cached', '--name-status', '--no-renames', '-z', 'HEAD', '--', '.agents/rules', '.agents/skills',
  ]);
...
export async function detectBranchResourceChanges(
  ...
  const diffOutput = await runGit(workspacePath, [
    'diff', '--name-status', '--no-renames', '-z', `${baseReference}...HEAD`, '--', '.agents/rules', '.agents/skills',
  ]);
  const resources = mergeResourceChanges(parseNameStatus(diffOutput));
  return Promise.all(resources.map(async ({ kind, name }) => {
    const resource = { kind, name };
