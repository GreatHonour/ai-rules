...
import type { LockedPackage, ProjectLock } from './types.js';
...
async function copyLockedPackage(
  managedRoot: string,
  registryRoot: string,
  ...
  if (lockedPackage.kind === 'rule') {
    ...
    if (claimedTargets.has(target)) throw new Error(`多个包写入同一目标: ${target}`);
...
export async function syncLockedPackages(options: SyncOptions): Promise<void> {
  const managedRoot = path.join(options.workspace, '.agentctl', `managed-${process.pid}-${Date.now()}`);
  await mkdir(managedRoot, { recursive: true });
  const checkoutCache = new Map<string, Awaited<ReturnType<typeof checkoutLockedRegistry>>>();
