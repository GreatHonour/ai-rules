...
export async function uploadIssues(workspacePath: string, issueName: string, remoteName = 'origin'): Promise<IssueUploadResult> {
  ...
  try {
    ...
    const stagedPathsText = await runGit(workspacePath, ['diff', '--cached', '--name-only', '-z'], {
    ...
    const commitHash = (
      await runGit(workspacePath, ['commit-tree', treeHash, '-p', headCommit, '-m', `docs(issue): ${issueName}`])
    ).trim();
    const commitPathsText = await runGit(workspacePath, ['diff-tree', '--no-commit-id', '--name-only', '-r', '-z', commitHash]);
