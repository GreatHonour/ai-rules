...
export async function configureProject(
  ...
  const selectedRules = Object.fromEntries(options.ruleNames.map((ruleName) => {
  ...
  const requests: readonly ResourceDownloadRequest[] = Object.keys(selectedRules)
    .map((name) => ({ kind: 'rules', name, repositoryUrl: registry.repositoryUrl }));
  ...
  try {
    await syncResources(options.workspacePath, downloadedBatch.resources, {
      ...
      afterSwap: async () => writeAgentsDocument(options.workspacePath, Object.keys(selectedRules)),
    });
    return nextManifest;
  } finally {
