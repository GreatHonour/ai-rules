...
export async function publishPackage(options: PublishOptions): Promise<void> {
  ...
  if (files.length === 0) throw new Error('发布源目录为空');
  ...
  if (await pathExists(finalPackageRoot)) throw new Error(`版本已存在: ${options.version}`);
  ...
  const versionDefinition: RegistryPackageVersion = {
  ...
  try {
    ...
    const current: RegistryIndex = await pathExists(indexPath)
    ...
    try {
      ...
      if (await pathExists(finalPackageRoot)) await rm(finalPackageRoot, { recursive: true, force: true });
      throw error;
    }
  } finally {
  ...
  if (options.commit) {
    ...
    if (options.push) await runGit(['push'], options.registryDirectory);
  }
}
