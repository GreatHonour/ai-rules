...
export async function checkProject(workspace: string): Promise<CheckResult> {
  ...
  try {
    ...
    const checkouts = new Map<string, Awaited<ReturnType<typeof checkoutLockedRegistry>>>();
...
export async function isInitialized(workspace: string): Promise<boolean> {
