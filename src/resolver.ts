...
import type { LockedPackage, ProjectLock, ProjectManifest } from './types.js';
...
export async function resolveManifest(workspace: string, manifest: ProjectManifest): Promise<ProjectLock> {
  const lockedPackages: LockedPackage[] = [];
  const registryCheckouts = new Map<string, Awaited<ReturnType<typeof checkoutRegistry>>>();
...
export function assertLockMatchesManifest(manifest: ProjectManifest, lock: ProjectLock): void {
  const expected = new Set<string>();
  for (const name of Object.keys(manifest.dependencies.rules)) expected.add(`rule:${name}`);
  for (const name of Object.keys(manifest.dependencies.skills)) expected.add(`skill:${name}`);
...
export function packageRoot(registryRoot: string, packagePath: string): string {
