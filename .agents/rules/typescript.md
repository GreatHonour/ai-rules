...
联合字面量类型**先独立导出为 `type` 别名**，再在 `interface` 中引用：
...
非响应式常量、函数入参 → `readonly` / `as const`（Vue 响应式数据不受此限制）
...
- 禁止全局可变状态 — 用工厂函数 + 闭包替代模块级 `let`
- 禁止 `as unknown as` 双重断言 — 类型不匹配应修正类型定义
- 类型定义与实际数据必须一致 — 后端返回 `string | string[]` 就不能只写 `string[]`
