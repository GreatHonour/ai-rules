...
| OpenAPI `integer(int64)` 前端统一映射为 `string` | 避免大整数精度风险，接口类型与页面类型都按字符串处理 |
| 禁止过度拆分 | 逻辑内聚的代码块不要强行抽函数；仅当被复用 ≥2 次**或**函数体超过 30 行时才提取 |
...
- 参数超过 2 个 → 合并为对象参数并定义 interface
- 异步函数统一 `async/await` + `try/catch`，**禁止裸跑 Promise**
...
| 2 | 函数单一职责 — 大函数必须按职责拆分，禁止一坨到底 | |
| 3 | 参数传对象 — 能传整个对象就不要拆成散装字段 | |
| 4 | 导出函数 / 工厂函数必须标注返回类型 | ⚠️ 屡犯，见正文 |
| 5 | API 泛型不能裸跑 — `post<void>()` 禁止裸 `post()` | ⚠️ 屡犯，见正文 |
| 6 | 禁止 `as unknown as` 双重断言 — 类型不匹配应修正类型 | |
| 7 | 类型定义与实际数据必须一致 — 后端返回 `string \| string[]` 就不能只写 `string[]` | |
