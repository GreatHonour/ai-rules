...
2. **设计范围**：一份 `design.md` 覆盖本次交付和全部已确认决策；规模无法在一份文档内保持清晰时，返回 `brainstorm` 拆分需求。
...
- UI 组件：Props、Events、Slots、交互状态、视觉约束、可访问性
- API/服务：请求响应、错误语义、权限、限流、幂等
- 数据变更：Schema、迁移、兼容、回滚
- 异步流程：状态机、超时、重试、补偿、可观测性
- ToC 活动：资格、时间、次数/金额、概率、库存、发放、撤销、风控
