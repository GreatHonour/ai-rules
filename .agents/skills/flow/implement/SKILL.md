...
1. 实现前读取 `AGENTS.md`、相关规则和现有代码，沿用项目约定。
2. 按 `design.md` 和任务边界实现，不增加无关功能或重构。
3. 可测试的行为变更使用 TDD：先确认测试失败，再实现并确认通过。文档、配置、静态样式等无法有效测试的任务使用对应检查，不伪造测试。
4. 测试、类型检查、lint 和 build 使用项目已有命令，只执行适用项。
5. 需要改变业务行为、边界或方案时停止并报告。实现级修正同步到 `design.md`；任务边界变化同步到 `task.md`。
...
6. 调用 `review/SKILL.md`；如有问题，修复后重新验证受影响任务，最多重试 2 次后报告剩余阻塞项。review 通过后由 review 负责路由至 e2e 或 archive，implement 流程结束。
