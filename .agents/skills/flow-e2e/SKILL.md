...
**执行流程：** 确定范围 → 选择工具 → 能力预检 → 执行验证 → 处理失败 → 输出报告 → 流转下游
...
- 交互后等待页面状态不再变化（DOM 无更新且网络请求完成），再以可见文本、DOM、URL 或控制台日志记录证据；命令成功不能单独作为证据
...
按 Adapter 指引重置 viewport、关闭页面并删除临时脚本或截图。
