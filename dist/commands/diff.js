...
export async function describeDiff(workspace) {
    const lock = await readLock(workspace);
    const state = await readState(workspace);
    const messages = lock.packages.map((lockedPackage) => `${lockedPackage.kind}:${lockedPackage.name}@${lockedPackage.version} ${lockedPackage.commit.slice(0, 12)}`);
    if (state === undefined) {
        ...
        for (const file of [...allFiles].sort((left, right) => left.localeCompare(right))) {
            if (state.files[file] === undefined)
                messages.push(`托管新增: ${file}`);
            else if (currentFiles[file] === undefined)
                messages.push(`托管删除: ${file}`);
            else if (state.files[file] !== currentFiles[file])
                messages.push(`托管修改: ${file}`);
        }
    }
    for (const scope of ['overrides', 'overrides.local']) {
...
async function readCurrentFileHashes(workspace) {
    const results = {};
    for (const directory of ['rules', 'skills']) {
        const hashes = await hashFiles(path.join(workspace, '.agents', directory));
        for (const [file, hash] of Object.entries(hashes))
            results[`${directory}/${file}`] = hash;
    }
    return results;
}
